...
const PEPPERFRY_WAREHOUSES = ['Gurgaon', 'Kolkata', 'New Bangalore', 'Bhiwandi'];
...
function generateOtp() {
  // Cryptographically random 6-digit OTP, unique every time
...
// ── SUBMIT CREATE ORDER ───────────────────────────────────────────────────────
// Step 1: Validate → generate OTP → send WhatsApp → show OTP dialog.
// Step 2: User enters OTP → verify → place order (Step 2 handled by confirmOtp()).
async function submitCreateOrder(e) {
  ...
  var otp = generateOtp();
  storeOtp(otp);
  _pendingPayload = payload;  // save payload so confirmOtp() can use it

  // Build summary values for the WhatsApp message
  var customerName  = (payload.customer[0].billing.name)  || 'Customer';
  ...
  try {
    ...
    if (!otpRes.ok || !otpData.success) {
      throw new Error(otpData.message || 'Failed to send OTP via WhatsApp.');
    }
    ...
    showToast('error', 'OTP Error', err.message);
  }
}
...
function showOtpDialog(orderNumber) {
  ...
  setTimeout(function () { document.getElementById('otpInput').focus(); }, 150);
}
...
// Allow pressing Enter inside the OTP input
...
async function confirmOtp() {
  ...
  // OTP matched — close dialog and place the order
  document.getElementById('otpModal').classList.remove('open');
  clearOtp();
